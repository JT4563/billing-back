...
export const summary = async (req, res) => {
  try {
    ...
    const [tm] = await Invoice.aggregate(
      sumAgg({ ownerId, createdAt: { $gte: tmStart, $lte: tmEnd } })
    );
    const [pm] = await Invoice.aggregate(
      sumAgg({ ownerId, createdAt: { $gte: pmStart, $lte: pmEnd } })
    );
    const [ty] = await Invoice.aggregate(
      sumAgg({ ownerId, createdAt: { $gte: tyStart, $lte: tyEnd } })
    );
